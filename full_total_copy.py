...
new_data =0        
def go_to_search_for(Money, index):
    ...
    if link_column is not None:
        ...
        for index, shareable_link in df.iloc[2:, df.columns.get_loc(link_column)].items():
            if pd.isna(shareable_link):
             print(f"Skipping empty link at index {index+2}")
             continue  # Skip empty links
            if not exists(shareable_link):
             print(f"Skipping inaccessible link at index {index + 2}")
             print(f"{shareable_link}")
             continue
            if contains_folder_in_google_drive(shareable_link):
                print(f"folder is in the link at index {index + 2}")
                continue
            if restricted_or_not_pdf(shareable_link):
                print("restricted")
                continue
            if check_link_in_set(shareable_link,empty_set):
                print(f"This link is duplicate  at {index + 2} as it is present at the excel in the above column")
                continue
                ...
                if pd.isna(column_data):
                  ...
                  target_word = "DATA IS NOT THERE IN EXCEL"
                    ...
                    if column_name == Money :
                        integer_part, decimal_part = str(column_data).split('.')
        ...
        print(f"{true_count}")
        print(f"system have detected Money_data in {year_range1} : {Money_data1}") 
        print(f"system have detected Money_data in {year_range2}: {Money_data2}") 
        print(f"system have detected Money_data in {year_range3}: {Money_data3}") 
        print(f"system have detected Money_data in {year_range4}: {Money_data4}") 
        print(f"system have detected Money_data in {year_range5}: {Money_data5}") 

        print(f"you have enetered Money_data in {year_range1} :{Lakhs7}") 
        print(f"you have enetered Money_data in {year_range2}: {Lakhs8}") 
        print(f"you have enetered Money_data in {year_range3}: {Lakhs9}") 
        print(f"you have enetered Money_data in {year_range4}: {Lakhs10}") 
        print(f"you have enetered Money_data in {year_range5}: {Lakhs11}") 
